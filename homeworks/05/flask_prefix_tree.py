...
class PrefixTree:
	...
	def check(self,string):
		if string == '':
			return False
		if string == None:
			raise ValueError('vvedite stroku')
		now = self.head
		exists = True
	...
	def check_part(self,string):
		if string == '':
			return False
		if string == None:
			raise ValueError('vvedite chto-to')
		now = self.head
		exists = True
		for letter in string:
			if letter in now.children:
...
init_prefix_tree('metro.txt',tree)
tree.getTop10('п')
...
def return_sudgest(string):
	...
	result = '<br>Станции метро по запросу:  <b>' + string + '</b>:<br>'
	for item in list_:
		result += '<br>'
		result += item
	result += '<br>'
